...
**Notlar: (Frontend v1.0.0'dan farklı olarak)**
- `refreshToken` içerir.
...
**Notlar: (Frontend v1.0.0'dan farklı olarak)**
- Randevular artık `data` yerine `data.items` altında.
- Yeni alanlar: `current_page`, `total_pages`, `total_items`.
...
**Notlar: (Frontend v1.0.0'dan farklı olarak)**
- `userId` isteğe bağlı yapıldı. Yalnızca admin gözünde bir değeri olacak.
- Mesajlar artık `data` yerine `data.items` altında.
- Yeni alanlar: `current_page`, `total_pages`, `total_items`.
...
```json
{
  ...
  "data": [
    ...
    {
      "...": "Thread altındaki diğer mesajlar (kronolojik sırada)"
    }
  ]
}
```
...
**Query Parameters:**
- `userId`* (optional) - Gönderen kullanıcı ID'si
- `receiverId` (required) - Alıcı kullanıcı ID'si
- `subject` (optional) - Mesaj konusu
- `content` (required) - Mesaj içeriği
- `threadId` (optional) - Thread ID'si, yeni mesaj için `Null`
...
**Notlar: (Frontend v1.0.0'dan farklı olarak)**
- `userId` eklendi. Adminler farklı hesaplar adına mesaj gönderebilir.
...
**Notlar: (Frontend v1.0.0'dan farklı olarak)**
- `total` kaldırıldı. See `data.total_items` instead.
- Kullanıcılar artık `data` yerine `data.items` altında.
- Yeni alanlar: `current_page`, `total_pages`, `total_items`.
...
**Notlar:**
- Backendde henüz implemente edilmedi.
...
**Query Parameters:**
- `userId` (optional) - Kullanıcı ID'si. Yalnızca admin gözünde bir değeri olacak.
...
```
**Notlar:**
- `type` değerleri: `info`, `success`, `warning`, `error`  
- `targetAudience` değerleri: `all`, `student`, `academician`  
- `priority` değerleri: `low`, `medium`, `high`  
- `status` değerleri: `active`, `archived`

**Notlar: (Frontend v1.0.0'dan farklı olarak)**
- Duyurular artık `data` yerine `data.items` altında.
- Yeni alanlar: `current_page`, `total_pages`, `total_items`.
